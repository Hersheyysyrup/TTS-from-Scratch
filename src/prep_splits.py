...
def random_split_and_prep_ljspeech_metadata(path_to_data, path_to_save= "data/", seed =42, test_split_pct= 0.01, sort = True):
    ...
    #Normalized Transcription: transcription with numbers, ordinals, and monetary unit expanded into full words (UTF-8)
    metadata = metadata[["file_path", "normalized_transcript"]]

    #Couple of rows with nan for normalized transcriptys, remove them ###
    ...
    ###Sort from longest to shortest so patting is minimized ###
    if sort:
